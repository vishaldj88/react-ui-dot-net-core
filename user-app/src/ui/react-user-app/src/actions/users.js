...
export const createUser = (firstName, lastName,emailId,gender,status) => async (dispatch) => {
  try {
    const res = await UserDataService.create({ firstName, lastName,emailId,gender,status });
...
export const findByFirstName = (firstName) => async (dispatch) => {
  try {
    const res = await UserDataService.findByFirstName(firstName);
